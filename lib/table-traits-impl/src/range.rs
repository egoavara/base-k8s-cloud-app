...
impl<T: InputType> InputType for Range<T> {
    ...
    fn parse(value: Option<async_graphql::Value>) -> async_graphql::InputValueResult<Self> {
        if let Some(async_graphql::Value::Object(obj)) = value {
            let mut min: T = async_graphql::InputType::parse(obj.get("min").cloned())
                .map_err(async_graphql::InputValueError::propagate)?;

            let mut max: T = async_graphql::InputType::parse(obj.get("max").cloned())
                .map_err(async_graphql::InputValueError::propagate)?;
            let obj = Self { min, max };
            Ok(obj)
        } else {
            Err(async_graphql::InputValueError::expected_type(
    ...
    fn to_value(&self) -> async_graphql::Value {
        ...
        map.insert(Name::new("min"), InputType::to_value(&self.min));
        map.insert(Name::new("max"), InputType::to_value(&self.max));
